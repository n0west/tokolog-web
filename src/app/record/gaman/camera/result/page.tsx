...
function GamanCameraResultPageContent() {
  ...
  // OCR処理を実行する関数
  const performOCR = async (imageDataUrl: string, testModeEnabled: boolean = false) => {
    setOcrState({
      isProcessing: true,
      progress: 10,
      stage: 'uploading',
    ...
    try {
      ...
      // OCR処理を実行（テストモードを含む）
      const ocrResponse = await processImageWithOCR(imageDataUrl, testModeEnabled);
      ...
      if (ocrResponse.success) {
        ...
        if (testModeEnabled && (ocrResponse as any).testMode) {
          ...
          // 通常モード：OCR結果をCameraResultData形式に変換（ガマンタイプ）
          const convertedData = convertOCRToCameraData(ocrResponse, 'gaman');
  ...
  const retryOCR = () => {
    if (imageData) {
      performOCR(imageData, isTestMode);
    }
  };
  ...
  const handleTestModeToggle = () => {
    ...
    // 画像データがある場合は自動的に再実行
    if (imageData) {
      performOCR(imageData, newTestMode);
    }
  };
  ...
  useEffect(() => {
    ...
    if (fromEdit !== 'true') {
      ...
      performOCR(savedImage, isTestMode);
    }
  }, [router, isTestMode]);
  ...
  useEffect(() => {
    ...
    console.log('🔍 [GAMAN] useEffect triggered - URLパラメータ確認:', {
      fromEdit,
      updatedItemId,
      deletedItemId,
    ...
    if (fromEdit === 'true' && updatedItemId) {
      ...
      console.log('📦 [GAMAN] セッションストレージの cameraEditData:', cameraEditData);
          ...
          setExtractedData(prev => {
            ...
            if (isNewItem) {
              ...
              if (existingIndex === -1) {
                ...
                console.log('➕ [GAMAN] 新規項目を追加しました:', editedData.description);
              } else {
                // 既存の新規項目を更新
                updated = baseData.map((item, index) => 
                  index === existingIndex 
                    ? {
                        ...
                        amount: editedData.passed_amount || item.amount,
                        memo: editedData.memo || item.memo || '',
                        updatedAt: new Date().toISOString(),
                      }
                    : item
                );
                console.log('✅ [GAMAN] 新規項目を更新しました:', editedData.description);
              }
            } else {
              // 既存項目の更新（OCR項目）
              console.log('🔄 [GAMAN] 既存項目更新 - 元ID:', originalId, '更新ID:', updatedItemId);
              ...
              if (foundItem) {
                console.log('✅ [GAMAN] 既存項目を更新しました:', editedData.description);
              } else {
                console.log('⚠️ [GAMAN] 更新対象の項目が見つかりません:', originalId);
              }
            }
    ...
    if (fromEdit === 'true' && deletedItemId) {
      console.log('🗑️ [GAMAN] 削除処理開始:', deletedItemId);
      const cameraEditData = sessionStorage.getItem('cameraEditData');
      console.log('📦 [GAMAN] 削除用セッションデータ:', cameraEditData);
      
      if (cameraEditData) {
        try {
          const editedData = JSON.parse(cameraEditData);
          console.log('✅ [GAMAN] 削除データ解析:', editedData);
          ...
          if (editedData.isDeleted) {
            ...
            console.log('🔄 [GAMAN] 削除IDマッピング - 元ID:', originalId, '削除ID:', deletedItemId);
            ...
            if (currentSessionData) {
              try {
                ...
                if (sessionParsed.extractedData && Array.isArray(sessionParsed.extractedData)) {
                  ...
                  console.log('💾 [GAMAN] 削除用データ取得元: セッションストレージ (', sourceData.length, '項目)');
                } else {
                  console.log('💾 [GAMAN] 削除用データ取得元: React状態 (', sourceData.length, '項目)');
                }
              } catch (error) {
                console.error('[GAMAN] セッションデータパース失敗、React状態を使用:', error);
              }
            } else {
              console.log('💾 [GAMAN] 削除用データ取得元: React状態のみ (', sourceData.length, '項目)');
            }
            ...
            console.log('🔢 [GAMAN] 削除処理 - 削除前:', beforeCount, '項目 → 削除後:', afterCount, '項目');
            console.log('🎯 [GAMAN] 削除対象ID:', originalId, '削除成功:', beforeCount > afterCount);
            ...
            console.log(`✅ [GAMAN] アイテム ${deletedItemId} を削除しました`);
          }
          ...
          console.error('[GAMAN] 削除データの反映に失敗:', error);
          alert('削除の反映に失敗しました。画面を再読み込みしてください。');
          // エラー時は画面をリフレッシュして最新状態に戻す
          window.location.reload();
        }
      } else {
        console.warn('[GAMAN] 削除データが見つかりません:', deletedItemId);
      }
    }
  }, [searchParams]);
  ...
  const handleAddNewItem = () => {
    ...
    // 3. 編集ページに遷移
    router.push(`/edit-camera/${tempId}?type=gaman`);
  };
  ...
  const handleNavigateToEdit = (id: string, data: { amount: number; productName: string; type: string }) => {
    ...
    const editData = {
      ...
      amount: 0, // ガマンの場合は0
      discount_amount: 0,
      passed_amount: data.amount, // ガマンの場合はpassed_amountに設定
  ...
  const handleRegisterAll = async (data: CameraResultData[]) => {
    ...
    try {
      ...
      const insertPromises = data.map(async (item) => {
        ...
        console.log('ガマン登録データ:', insertData);
...
export default function GamanCameraResultPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <GamanCameraResultPageContent />
    </Suspense>
  );
}